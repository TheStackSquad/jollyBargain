...
export const adminLogin = async (identifier, password) => {
  try {
    ...
    if (response.data && response.data.data && response.data.data.token) {
      ...
      throw new Error(response.data.message || 'Login failed: No token received or unexpected response structure.');
    }
  } catch (error) {
...
export const isAuthenticated = () => {
  ...
  console.log('authService: Checking isAuthenticated. Token in localStorage:', !!token); // Log token presence
  return !!token;
};
