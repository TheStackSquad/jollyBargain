...
const Dashboard = () => {
  ...
  const navItems = [
    { id: 'overview', name: 'Dashboard', icon: LayoutDashboard, component: <DashboardOverview userName={userName} /> },
    { id: 'orders', name: 'My Orders', icon: ShoppingBag, component: <OrderHistory /> },
    { id: 'settings', name: 'Account Settings', icon: User, component: <AccountSettings /> },
  ...
  return (
    ...
    >
      ...
      <motion.nav
        ...
        className="w-full lg:w-64 bg-white shadow-lg lg:shadow-xl p-4 lg:p-6 flex flex-col justify-between rounded-b-xl lg:rounded-r-xl lg:rounded-bl-none"
      >
        <div>
          <h1 className="text-3xl font-extrabold text-blue-600 mb-8 text-center">My Account</h1>
          <ul className="space-y-3">
            {navItems.map((item) => (
              <li key={item.id}>
                <button
                  onClick={() => setActiveTab(item.id)}
                  className={`flex items-center w-full px-4 py-3 rounded-lg text-lg font-medium transition-all duration-300 ease-in-out
                    ${activeTab === item.id
        ...
        <div className="mt-8">
          <button
            ...
            className="flex items-center w-full px-4 py-3 rounded-lg text-lg font-medium text-red-600 bg-red-50 hover:bg-red-100 transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
          >
      ...
      >
        <div className="bg-white p-6 rounded-xl shadow-xl min-h-[calc(100vh-2rem)] lg:min-h-[calc(100vh-4rem)]">
