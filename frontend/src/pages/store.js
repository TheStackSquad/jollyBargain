...
const ShopNowPage = () => {
  ...
  const [error, setError] = useState(null); // Custom hooks
  ...
  const { wishlist, toggleWishlist } = useWishlist(); // Load initial data
  ...
  useEffect(() => {
    ...
    loadInitialData();
  }, [loadProducts]); // Dependency added for loadProducts // Apply filters when they change
  ...
  useEffect(() => {
    ...
    applyFilters();
  }, [filters, loadProducts]); // Dependencies added for filters and loadProducts // Handle load more - using useCallback for memoization
  ...
  }, [currentPage, loadMoreProducts, filters]); // Handle add to cart with feedback - using useCallback for memoization

  const handleAddToCart = useCallback(
    (product) => {
      addToCart(product); // You can add toast notification here
  ...
  const handleWishlistToggle = useCallback(
    (productId) => {
      toggleWishlist(productId); // You can add toast notification here
    },
    [toggleWishlist]
  );

  if (isLoading && filteredProducts.length === 0) {
    // Using filteredProducts here for consistency
    return (
