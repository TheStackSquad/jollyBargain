...
const CartPage = () => {
  ...
  const [vatRate, setVatRate] = React.useState(0.075); // Realistic VAT rate for Nigeria (7.5%)
  const [discountAmount, setDiscountAmount] = React.useState(0); // New state for applied discount

  // Define available coupons (for client-side simulation)
  const availableCoupons = {
    'SAVE10': { type: 'percentage', value: 0.10, minSubtotal: 5000 }, // 10% off for orders over $5000
  ...
  const { subtotal, totalVat, grandTotal } = useMemo(() => {
    ...
    const calculatedGrandTotal = subtotalAfterDiscount + calculatedVat + shippingCost;
  ...
  }, [cartItems, shippingCost, vatRate, discountAmount, couponCode]); // Add couponCode to dependencies
  ...
  const handleApplyCoupon = () => {
    ...
    if (coupon) {
      if (subtotal < coupon.minSubtotal) {
        setCouponMessage({ type: 'error', text: `Coupon requires a subtotal of $${coupon.minSubtotal.toFixed(2)}.` });
        setDiscountAmount(0);
        return;
      }
      ...
      if (coupon.type === 'percentage') {
        ...
        setCouponMessage({ type: 'success', text: `Coupon "${code}" applied! ${coupon.value * 100}% off.` });
      } else if (coupon.type === 'fixed') {
        newDiscount = coupon.value;
        setCouponMessage({ type: 'success', text: `Coupon "${code}" applied! $${coupon.value.toFixed(2)} off.` });
      } else if (coupon.type === 'shipping') {
        setShippingCost(0); // Set shipping to 0 for free shipping
        setShippingMessage({ type: 'success', text: 'Free shipping applied!' });
        setCouponMessage({ type: 'success', text: `Coupon "${code}" applied! Free shipping.` });
      }
  ...
  const handleCheckout = () => {
    ...
    // In a real application, you might perform final client-side validation
    // and then send the cart data to a backend API to create an order,
    // reserve stock, and get final server-calculated totals.
    // Upon successful backend response, you would then navigate.
    // For this example, we'll directly navigate.
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 font-inter text-gray-800 p-4 sm:p-6 lg:p-8">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-20 left-1/4 w-40 h-40 bg-pink-200 rounded-full opacity-20 animate-pulse delay-2000"></div>
      </div>
                ...
                <div className="lg:col-span-2">
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      <div>
                        <h2 className="text-2xl font-bold text-gray-800">Items in your Cart</h2>
