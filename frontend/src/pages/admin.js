...
function AdminPage() {
  ...
  const handleDeleteClick = (productId) => {
    // console.log(
    //   `AdminPage: Delete button clicked for product ID: ${productId}`,
    // );
    // Find the product to display its details in the modal
    // const product = products.find((p) => p._id === productId);
    const product = products.find((p) => {
      const { _id: id } = p; // Renames _id to id
      return id === productId;
    });
  ...
  const handleConfirmDelete = async () => {
    // console.log(
    //   `AdminPage: User confirmed deletion for product ID: ${deletingId}`,
    // );
    if (deletingId) {
  ...
  if (!isAuthenticated) {
    return (
      <motion.div initial="hidden" animate="visible" variants={pageMountAnim}>
        <AdminLoginForm
          onAuthenticate={(id, pw) =>
            handleAdminAuthenticate(id, pw, setIsAuthenticated, setAuthError)
          }
          error={authError}
        />
      </motion.div>
