...
const FlashDeals = () => {
  ...
  const filterDeals = (deals, filters) => {
    return deals.filter(deal => {
      if (filters.searchTerm && !deal.title?.toLowerCase().includes(filters.searchTerm.toLowerCase())) return false;
      if (filters.category !== 'all' && deal.category?.toLowerCase() !== filters.category.toLowerCase()) return false;
      if (filters.priceRange.min && deal.price < parseInt(filters.priceRange.min)) return false;
      if (filters.priceRange.max && deal.price > parseInt(filters.priceRange.max)) return false;
      return true;
    });
  };
  ...
  const handleClaimDeal = (deal) => {
    alert(`Claiming deal: ${deal.title} for ${new Intl.NumberFormat('en-NG', {
      style: 'currency',
      currency: 'NGN',
