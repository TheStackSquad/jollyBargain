...
const Wishlist = () => {
  ...
  return (
    <motion.div
      ...
      className="space-y-6"
    >
      <h2 className="text-4xl font-extrabold text-gray-900 mb-6">My Wishlist</h2>

      {wishlistItems.length === 0 ? (
        <p className="text-gray-600">Your wishlist is empty. Start adding some items!</p>
      ) : (
        <motion.div variants={staggerContainer(0.1, 0.2)}>
          {wishlistItems.map((item, index) => (
            <motion.div
              ...
              className="flex items-center bg-white p-4 rounded-xl shadow-md border border-gray-200 mb-4 last:mb-0"
            >
              <img
                src={item.imageUrl}
                alt={item.name}
              ...
              <div className="flex flex-col sm:flex-row gap-2 ml-4 flex-shrink-0">
                ...
                <button
                  ...
                  className="bg-red-500 text-white py-2 px-4 rounded-lg shadow hover:bg-red-600 transition-colors duration-200 ease-in-out text-sm"
                >
                  Remove
                </button>
              </div>
