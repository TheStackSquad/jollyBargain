...
const ProductPreview = ({ product }) => {
  ...
  const hasContent = displayProduct.title && displayProduct.title !== 'Product Title';
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      ...
      <div className="space-y-4">
        {/* Image Preview */}
        {displayProduct.images && displayProduct.images.length > 0 && displayProduct.images[0]?.url ? (
        ...
        <div>
          <h4 className={`font-medium text-lg ${hasContent ? 'text-gray-900' : 'text-gray-500'}`}>
        ...
        <div className="text-sm text-gray-600 space-y-2">
          <p>
            ...
            <span className={displayProduct.category ? 'text-gray-900' : 'text-gray-500'}>
          ...
          <p>
            ...
            <span className={`ml-1 ${formatStock(displayProduct.stock) > 0 ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <p>
            ...
            <span className={`ml-1 px-2 py-1 rounded-full text-xs ${
              displayProduct.status === 'active' ? 'bg-green-100 text-green-800' :
              displayProduct.status === 'draft' ? 'bg-yellow-100 text-yellow-800' :
        ...
        <div>
          ...
          <p className={`text-sm ${displayProduct.description && displayProduct.description !== 'Product description will appear here.' ? 'text-gray-700' : 'text-gray-500'}`}>
            {displayProduct.description || 'Product description will appear here.'}
          </p>
        </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {parseTags(displayProduct.tags).length > 0 ? (
                parseTags(displayProduct.tags).map((tag, index) => (
                  <span
                    key={index}
