...
const paymentSchema = new mongoose.Schema(
  {
    ...
    paymentMethodType: {
      ...
      description: 'The type of payment method used (e.g., credit_card, paypal).',
    },
    paymentToken: {
      type: String,
      required: true,
      description: 'A token representing the payment method, provided by a payment gateway. NEVER store raw card details.',
    },
    transactionId: {
      type: String,
      unique: true,
      sparse: true, // Allows multiple documents to not have this field, but if present, it must be unique
      description: 'The unique ID returned by the payment gateway for this transaction.',
    },
    status: {
      type: String,
      required: true,
