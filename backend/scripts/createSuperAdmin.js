...
const createSuperAdminUser = async () => {
  try {
    ...
    if (existingUser) {
      console.warn(`⚠️ User with email "${existingUser.email}" or username "${existingUser.username}" already exists.`);
      // If an existing user is found and is already a superadmin, we can exit.
      if (existingUser.role === 'superadmin') {
        console.log('Super Admin user already exists. No new super admin created.');
        rl.close();
        process.exit(0);
      } else {
        ...
        console.error('❌ A user with this email or username already exists but is not a Super Admin. Please use different credentials or manually update their role.');
        rl.close();
        process.exit(1);
      }
    }
