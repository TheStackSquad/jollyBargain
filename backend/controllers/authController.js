...
const registerUser = asyncHandler(async (req, res) => {
  ...
  console.log('[Auth Controller] Registering user with:', { email, name: name || 'N/A' });
  ...
  if (user) {
    console.log('[Auth Controller] User registered successfully. User ID:', user._id);
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!identifier || !password) {
    console.log('[Auth Controller] Login failed: Missing identifier or password.');
    res.status(400);
  ...
  console.log('[Auth Controller] User found:', user ? user.email : 'No user found');

  if (user && (await user.matchPassword(password))) {
    console.log('[Auth Controller] Password matched successfully for user:', user.email);
    res.json({
      success: true,
    ...
    console.log('[Auth Controller] Login failed: Invalid credentials (user not found or password mismatch).');
