...
export const getFlashDeals = async (req, res) => {
  try {
    ...
    console.log('Attempting to fetch products with stock <= threshold and status "active"...');
    ...
    const transformedDeals = allDeals.map(deal => {
      ...
      return {
        ...
        images: dealObj.images.length > 0 ? dealObj.images : [
          {
    ...
    const featured = transformedDeals[Math.floor(Math.random() * transformedDeals.length)];
